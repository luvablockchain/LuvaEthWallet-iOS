...
extension UIImageView {
    ...
    open func setImage(string: String?, color: UIColor? = nil, circular: Bool = false, textAttributes: [NSAttributedString.Key: Any]? = nil) {
        ...
        let image = imageSnap(text: string != nil ? string?.contactInitials : "", color: color ?? UIColor.random, circular: circular, textAttributes: textAttributes)
...
extension UIImageView {
    ...
    public func setImageForAddress(string: String, backgroundColor: UIColor? = nil, circular: Bool, textAttributes: [NSAttributedString.Key: AnyObject]? = nil, gradient: Bool = false, gradientColors: GradientColors? = nil) {
        let initials: String = twoEndCharacterFromStringUppercased(string: string)
        let color: UIColor = (backgroundColor != nil) ? backgroundColor! : randomColor(for: string)
        let gradientColors = gradientColors ?? topAndBottomColors(for: color)
        let attributes: [NSAttributedString.Key: AnyObject] = (textAttributes != nil) ? textAttributes! : [
            NSAttributedString.Key.font: self.fontForFontName(name: nil),
    ...
    public func setImageForAddress1(string: String, backgroundColor: UIColor? = nil, circular: Bool, textAttributes: [NSAttributedString.Key: AnyObject]? = nil, gradient: Bool = false, gradientColors: GradientColors? = nil) {
        let initials: String = twoEndCharacterFromString(string: string)
        let color: UIColor = (backgroundColor != nil) ? backgroundColor! : randomColor(for: string)
        let gradientColors = gradientColors ?? topAndBottomColors(for: color)
        let attributes: [NSAttributedString.Key: AnyObject] = (textAttributes != nil) ? textAttributes! : [
            NSAttributedString.Key.font: self.fontForFontName(name: nil),
...
private func twoEndCharacterFromString(string: String) -> String {
    var initials = ""
    if(string.count >= 2) {
        let initialsArray = string.components(separatedBy: " ")
...
private func twoEndCharacterFromStringUppercased(string: String) -> String {
    if(string.count >= 2) {
        return string.suffix(2).uppercased()
    } else if(string.count == 1) {
